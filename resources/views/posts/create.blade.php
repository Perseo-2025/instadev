...
@section('contenido')
    <div class="md:flex md:items-center">
        <div class="md:w-1/2 px-10">
            ...
            <form method="POST" id="dropzone" action="{{ route('imagenes.store')}}" 
            enctype="multipart/form-data"
            class="dropzone border-dashed border-2 w-full h-96 rounded flex
            flex-xol justify-center items-center">
             @csrf
            </form>
        </div>
        <div class="md:w-1/2 px-10 bg-white p-10 rounded-lg box-shadow-xl mt-10 md:mt-0">
            <form action="{{ route('posts.store') }}" method="POST" novalidate>
                ...
                <div class="mb-5">
                    ...
                    <textarea
                        ...
                        name="descripcion"
                        type="text"
                        placeholder="Empieza una Descripción"
                        class="border p-3 w-full rounded-lg @error('name')
                            border-red-500
                        @enderror"
